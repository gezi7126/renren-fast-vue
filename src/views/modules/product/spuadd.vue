...
//////////////基本信息///////////////////////
    ...
    getMemberLevels() {
     ...
     //发布商品-》基本信息，选择商品分类后，获取商品分类关联的品牌
    getCheckedNodesPathFormChild(path) {
      //console.log("paths.....",path);
      //alert("vvvvv..........."+path);
      this.spu.catalogId = path[path.length - 1];
      this.catPathSub = PubSub.publish('catPath',path[path.length-1]);
    ...
    generateSaleAttrs() {
     ...
     getShowSaleAttr() {
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(
            `/product/attr/0/list/${this.spu.catalogId}`
          ),
          method: "get",
          params: this.$http.adornParams({
            page: 1,
